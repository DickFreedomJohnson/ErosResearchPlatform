...
/obj/item/device/radio/collar_shk
	name = "shock collar"
	desc = "For the noisy pet."
...
/obj/item/device/radio/collar_shk/Topic(href, href_list)
	//..()
	if(usr.stat || usr.restrained())
		return
	if(((istype(usr, /mob/living/carbon/human) && ((!( ticker ) || (ticker && ticker.mode != "monkey")) && usr.contents.Find(src))) || (usr.contents.Find(master) || (in_range(src, usr) && istype(loc, /turf)))))
...
/obj/item/device/radio/collar_shk/receive_signal(datum/signal/signal)
	if(!signal || signal.encryption != code)
		return
...
/obj/item/device/radio/collar_shk/attack_self(mob/user as mob, flag1)
